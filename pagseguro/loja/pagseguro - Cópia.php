...
$sql = "SELECT id_comp_usu FROM `tb_compra` WHERE cod_comp_usu = '$cod_comp_usu' && email_comp_usu = '$email_comp_usu'";
		$resultado = mysqli_query($conn, $sql) or die ("Erro ao consultar os dados na base.");
		if (mysqli_affected_rows($conn) != 0)
		{
		//COMPRA NO MERCADOPAGO
...
//SETA ID COMPRA FIM
		...
		else
		{
			...
			?>
<script type= \"text/javascript\">
function confPagSeguro(){ 
   if( !confirm("Essa ação não poderá ser revertida.\n Tem certeza que deseja comprar este produto do PagueSeguro. <?php $nome_comp_usu?>.?") )
           return false;
}
</script>            
<?php }//Else operação concluida ?>
<?php
}
else
{
