...
$nome = trim(filter_input(INPUT_GET, 'nomeCompra', FILTER_SANITIZE_STRING));
...
{ 
	if(isset($_POST['alterar']) && $_POST['alterar'] != null)
	{
		...
		if(isset($_POST['quantidadeProduto']) && $_POST['quantidadeProduto'] != null)
		{			
			$resultado = $dalProduto->localizarProdutoNome($nome);
			
			$dados = mysqli_fetch_array($resultado);
			
			$valUni = $dados['precoVenda'];
			$alt = $dados['altura'];
			$Larg = $dados['largura'];
			$compri = $dados['comprimento'];
			$pesinho = $dados['peso'];
			
			$valTotal = $valUni * $quantidadeProduto;
			$totAltura = $alt * $quantidadeProduto;
			$totLargura = $Larg * $quantidadeProduto;
			$totComprimento = $compri * $quantidadeProduto;
			$totPeso = $pesinho * $quantidadeProduto;
			
		$compra = new Compra($idCompra, '', '', $nome, '', $quantidadeProduto,'','','', $valTotal,'','','','','','','',$totAltura, $totLargura, $totComprimento, $totPeso);

		//print_r($compra);
		
		$cadastroOK = $dalCompra->alterarCompraQuantidade($compra);
...
<?php
	...
	$dados = mysqli_fetch_array($resultado);
	?>
<form name="formCadastroProduto" action="#" target="_self" method="post">
    <div class="container" align="center">
      <table class="form_menu2" align="center">
		  <input hidden="hidden" name="idCompra" type="text" id="nomeProduto" title="Nome do Produto" value='<?php echo $id;?>' size="40" maxlength=50">
        <tr>
		  <td align="center"><label for="nome">
              <img title="nome de usuário" class="rounded-circle" src="../../../images/1246548.png" width="35" height="37" /></label>&nbsp;<input class='btn btn-group border-0 text-center font-weight-bold' disabled="" name="nome" type="text" id="nome" placeholder="Informe o seu nome completo (Obrigatório)" title="Nome" value='<?php echo $dados['nomeCompra'];?>' size="20" maxlength="50">
		  &nbsp;</td><label for="nome">
          <td align="center"><h5><font color="#ff0000">*</font><label for="quantidadeProduto">
			  <img title="quantidadeProduto" class="rounded-circle" src="../../../images/1246548.png" width="35" height="37" />&nbsp;</label>
			  <input name="quantidadeProduto" class='btn btn-group border-0 text-center font-weight-bold' type="number" required="required" id="quantidadeProduto" placeholder="Informe a quantidade (Obrigatório)" max="100" min="1" title="Quantidade" value='<?php echo $dados['quantidadeProduto'];?>' size="13" maxlength=50">
		  </td>
        </tr>
      </table>
	<tr>
