...
function confDeletar(){ 
   if( !confirm("Esta operação é irreversível. Tem certeza que deseja deletar esta compra ?") )
           return false;
}
</script>
<?php session_start(); ?>
...
<?php include_once("../../Controller/DALFreteCorreios.php"); ?>
...
$dalFreteCorreios = new DALFreteCorreios($conexao);

/* AREA DE LOGIN QUE AINDA NÃO FUNCIONA
...
FIM AREA DE LOGIN QUE AINDA NÃO FUNCIONA
*/
//FIM POST PHP
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <body>
    ...
    <hr>
    <h1 align="center" class="text-center">CONFIRMAR COMPRA</h1>
    <hr>
    <div class="container">
...
<table align="center">
<tr>
	<td align="right"><h5>Nome: </h5></td><td align="center""><h5><?php echo $_SESSION['usuarioNome']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Email: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioEmail']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>DDD: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioTelDDD']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Telefone: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioTel']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Logradouro: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioEnd']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Complemento: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioCompl']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Bairro: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioBairro']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Cidade: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioCid']; ?></h5></td>
</tr>
<tr>
	<td align="right"><h5>Estado: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioEstado']; ?></h5></td>
</tr>
	<td align="right"><h5>CEP: </h5></td><td align="center"><h5><?php echo $_SESSION['usuarioCep']; ?></h5></td>
</tr>
<tr>
<td align="center"></td><td align="center"><a href='../Alterar/AlterarUsuarioCompra.php?idPessoa=<?php echo $dados['idPessoa'];?> '><input class="btn btn-primary" name='alterar' type='submit' value='alterar' ></a></td>
<tr>
</table>
<hr>
<table align="center">		
...
<form action="PagSeguro.php?idPessoa=<?php echo $dados['idPessoa'];?>" method="post" enctype="multipart/form-data" name="formConfirmarPagamento" id="form1" onsubmit="return conf();">
<hr>
<table align="center">
		<tr>
			<td align="center">
				<h5>Nº de Protutos: </h5>
		    </td>
			<td align="center">
				<h5>Nome: </h5>
		    </td>
	...
	$compra = new Compra(0, $_SESSION['usuarioId'], '', $_SESSION['usuarioNome'], $_SESSION['usuarioEmail'], '', '', '', '', $valorTotal = 0, $_SESSION['usuarioCPF'], 25, '', '', '', '', '');
	
	//print_r($compra);
	
	$resultado = $dalCompra->listarCompraUsuario($compra);
	...
	{ $contadorCarrinho = $contadorCarrinho + 1; ?>
        <tr>
		<!-- AREA DE SOMA DE CALCULO DE FRETE E CONTADOR DE PRODUTOS -->
		  <td align="center">
             <?php echo $contadorCarrinho;
	 					$totAltura += $dados['totalAltura'];
	 					$totLargura += $dados['totalLargura'];
	 					$totComprimento += $dados['totalComprimento'];
	 					$totPeso += $dados['totalPeso'];
	 					$totQuantidade += $dados['quantidadeProduto'];
			?>
         	<input hidden="" name="idProd" type="text" value='<?php echo $dados['idCompra'];?>' title="Peso" size="40" maxlength=50">
          </td>
		<!-- AREA DE SOMA DE CALCULO DE FRETE E CONTADOR DE PRODUTOS -->
          <td align="left">
              <img class="rounded-circle" src='<?php
	 		if(isset($dados['imagemCompra']) && $dados['imagemCompra'] != null)
			{
				echo "../../../imagens/".$dados['imagemCompra'];
			}
	 		else if($dados['imagemCompra'] == '')
		    {
				echo "../../../images/bkg06.jpg";
		    }
	 		else
		    {
				echo "../../../images/bkg06.jpg";
		    }
	 		?>'&nbsp;&nbsp;&nbsp;&nbsp;alt="Imagem do produto" width="43" height="33" id="imagem" title="Imagem do produto" />&nbsp;&nbsp;<?php echo $dados['nomeCompra']; ?>
          </td>
          <td align="center">
              <?php $quantidade = $dados['quantidadeProduto']; echo $dados['quantidadeProduto']; ?>
          </td>
		  <td align="center">
              R$ <?php $valUni = $dados['precoVenda'];  echo $dalCompra->virgula($valUni); ?>
          </td>
		  <td align="center">
              R$ <?php $valTotal = $quantidade * $valUni; $valCarrinho += $valTotal; $valTotal = $dalCompra->virgula(number_format($valTotal,2,".","")); echo($valTotal); //echo($dalCompra->virgula($valTotal)); ?>
          </td>
          <td align="center">
       		<a href='ConfirmarCompraAlterar.php?idProduto=<?php echo $dados['idCompra'];?> '><input class="btn btn-primary" name='alterar' type='button' value='alterar' ></a>
		  </td>
		<td align="center">
			  
            <a onclick="return confDeletar();" href='../Busca/ListaCompraDel.php?idProduto=<?php $idProduto = $dados['idProdutoCompra']; echo $dados['idCompra'];?> '><input class="btn btn-danger" name='deletar' type='button' value='Deletar' ></a>
		  </td><input hidden="" name="idCompra" value="<?php echo $dados['idCompra']; ?>"  />
        </tr>
		<!--
		  <td align="center">
			  
            <a onclick="return conf();" href='../Busca/ListaCompraDel.php?idProduto=<?php echo $dados['idCompra'];?> '><input class="btn btn-danger" name='deletar' type='submit' value='Deletar' ></a>
		  </td>
        </tr>
	-->
	  <?php } ?>
		<tr>
			<td></td><td></td><td></td><td><h5>Valor total carrinho: R$ </h5></td><td align="center"><h5><?php echo($dalCompra->virgula(number_format($valCarrinho,2,".",""))); ?></h5></td>
		</tr>
</table>
<hr>
<?php
	$totPeso = number_format($totPeso,2,".","");
	$valCarrinho = number_format($valCarrinho,2,".","");
	$avisoRecebmento ='n';
	$entregaMaos ='n';
	$codigoEmpresa= '';
	$senhaEmpresa= '';
	...
	//INFORMAR O CEP DO PROPRIETARIO DO SISTEMA PARA O CALCULO
		
	//FRETE CORREIOS ANTERIOR
	//$freteCorrios = new FreteCorreios(18,18,18,15,25,'27410200','11463190','','','n','n','40010');
	if($totAltura < 20)
	{
		$totAltura = 20;
	}
	...
	$totPeso = $dalProduto->virgula($totPeso);
		
	$freteCorrios = new FreteCorreios($totAltura, $totLargura, $totComprimento, $totPeso, $valCarrinho, $meuCEP, $_SESSION['usuarioCep'],$codigoEmpresa, $senhaEmpresa, $entregaMaos, $avisoRecebmento,'40010');
						
	//print_r($freteCorrios);
	
	$valSedex = $dalFreteCorreios->calcularFreteSedex($freteCorrios); echo($valSedex->valor);
		
	$valPac = $dalFreteCorreios->calcularFretePac($freteCorrios); echo($valPac->valor);
?>
<input hidden="" name="idCompra" type="text" value='<?php echo $idProduto;?>' title="Peso" size="40" maxlength=50">
<input hidden=""  name="totAltura" type="text" value='<?php echo $totAltura;?>' title="Peso" size="40" maxlength=50">
<input hidden="" name="totLargura" type="text" value='<?php echo $totLargura;?>' title="Peso" size="40" maxlength=50">
<input hidden="" name="totComprimento" type="text" value='<?php echo $totComprimento;?>' title="Peso" size="40" maxlength=50">
<input hidden="" name="totPeso" type="text" value='<?php echo $totPeso;?>' title="Peso" size="40" maxlength=50">
<input hidden=""  name="totQuantidade" type="text" value='<?php echo $totQuantidade;?>' title="Peso" size="40" maxlength=50">
<input hidden="" name="isCompra" type="text" value='<?php echo true;?>' title="Compra" size="40" maxlength=50">
<table align="center">		
<tr>
	<td><h4 align="center">FRETE</h4></td>
	<td align="left">
              <input class="btn btn-outline-success my-2 my-sm-0" name="tipoFrete" type="radio" id="Tipo frete" title="Sedex" value="S">
               SEDEX R$ <?php $sedex = $valSedex->Valor; $valTotsedex = $dalProduto->moeda($sedex) + $valCarrinho; echo($valSedex->Valor); ?><br>
              <input class="btn btn btn-outline-info my-2 my-sm-0" name="tipoFrete" type="radio" id="Tipo frete" title="Pac" value="P">
             PAC R$ <?php $pac = $valPac->Valor; $valTotPac = $dalProduto->moeda($pac) + $valCarrinho; echo($valPac->Valor); ?><br>
			<input name="tipoFrete" type="radio" id="Tipo frete" title="Em Maos" value="M">
             Retirar em mãos R$ 0,00</td>

		<td>&nbsp;&nbsp;&nbsp;</td>
	<td class="btn btn-outline-success my-2 my-sm-0" align="center">
              Valor total com frete<br>Sedex R$: <?php  echo($dalProduto->virgula($valTotsedex)); ?><br>* Prazo <?php  echo($valSedex->PrazoEntrega); ?> dias úteis
	</td>
	<td>&nbsp;&nbsp;</td>
	<td class="btn btn btn-outline-info my-2 my-sm-0" align="center">
             Valor total com frete<br>PAC R$: <?php echo($dalProduto->virgula($valTotPac)); ?><br>* Prazo <?php echo($valPac->PrazoEntrega); ?> dias úteis
	</td>
	<input hidden=""  name="valorSedex" value="<?php echo $valTotsedex; ?>"  />
	<input hidden=""  name="valorPac" value="<?php echo $valTotPac; ?>"  />
	<input hidden=""  name="valorMao" value="<?php echo $valCarrinho; ?>"  />
	<input hidden=""  name="valorFreteSedex" value="<?php echo $valSedex->Valor; ?>"  />
	<input hidden=""  name="valorFretePac" value="<?php echo $valPac->Valor; ?>"  />
</tr>
</table>
<hr>
<table align="center">
<table align="center">		
<tr>
	<td align="center"><input value="Compra carrinho" type="image" src="https://stc.pagseguro.uol.com.br/public/img/botoes/pagamentos/209x48-comprar-assina.gif" name="comprarPagSeguro" alt="Pague com PagSeguro - é rápido, grátis e seguro!" />
	</td>
</tr>
</table>
</form>
