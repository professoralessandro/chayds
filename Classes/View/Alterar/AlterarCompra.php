...
if(isset($_SESSION['usuarioAcessoNiveis']) && $_SESSION['usuarioAcessoNiveis'] != null && $_SESSION['usuarioAcessoNiveis'] == 'Gerente')
{
	...
	{
		$codigoRastreio = trim($_POST['codigoRastreio']);
		$idCompra = trim($_POST['idCompra']);
		$email = trim($_POST['email']);
		$nomeCliente = trim($_POST['nomeCliente']);
		
		$compra = new Compra($idCompra, '', '', '', '', '', '', '', '', '', '', '', '', '', $codigoRastreio);
		
		//print_r($compra);
		
		$alterarOK = $dalCompra->alterarCompraCodigoRasreio($compra);
		
		if(!$alterarOK)
		{
			mail($email, 'Seu produto CHAYDS acaba de ser postado.', '
			Parabéns'.$nomeCliente.';
			O seu produto acaba de ser postado nos correios,
			Gostaríamos de agradecer pela preferência e gostaríamos de avisar
			que a chayds está a sua disposição durante o processo de compra e
			pós venda do produto. Caso seja necessário entre na área de suporte
			ao cliente e nos envie uma mensagem com as dúvidas que  você tenha.
			...
			mail('professor_alessandro@hotmail.com', 'Seu produto CHAYDS acaba de ser postado.', '
			Parabéns'.$nomeCliente.';
			O seu produto acaba de ser postado nos correios,
			Gostaríamos de agradecer pela preferência e gostaríamos de avisar
			que a chayds está a sua disposição durante o processo de compra e
			...
			echo "<META HTTP-EQUIV=REFRESH CONTENT ='0;URL=http://www.chayds.com.br/Classes/View/Busca/ListaCompra.php'>
		...
		{
			echo "<META HTTP-EQUIV=REFRESH CONTENT ='0;URL=http://www.chayds.com.br/Classes/View/Alterar/AlterarCompra.php'>
			<script type= \"text/javascript\">
	...
	<?php
		...
		<hr>
		<h2 class="text-center">ALTERAR COMPRA</h2>
	<?php
			$id = trim(filter_input(INPUT_GET, 'idCompra', FILTER_SANITIZE_NUMBER_INT));

			$resultado = $dalCompra->localizarCompra($id);

			$dados = mysqli_fetch_array($resultado);
	?>
		<hr>
	<form name="formAlteraUsuario" action="#" target="_self" method="post">
		<div class="container" align="center">
		  <table class="form_menu2" align="center">
			<tr>
			  <td align="center"><h5>ID Compra: </h5>
				<font color="#000000">
				<blockquote>
				  <input hidden="" name="idCompra" type="text" id="id" placeholder="ID Compra" title="ID Compra" value='<?php echo $dados['idCompra'];?>' size="10" maxlength=50">
				  <input name="id" type="text" disabled="disabled" id="id" placeholder="ID Compra" title="ID Compra" value='<?php echo $dados['idCompra'];?>' size="10" maxlength=50">
			  </blockquote>
			  </td>
				<td align="center"><h5>Nome do produto: </h5>
				<font color="#000000">
				<blockquote>
				  <input name="nomeProduto" type="text" disabled="disabled" id="nomeProduto" placeholder="Informe o nome do produto (Obrigatório)" title="Nome do Produto" value='<?php echo $dados['nomeCompra'];?>' size="40" maxlength=50">
			  </blockquote>
			  </td>
			  <td align="center"><h5>Nome do cliente: </h5>
				<font color="#000000">
				<blockquote>
					<input hidden="" name="nomeCliente" type="text" id="nomeCliente" placeholder="Informe o nome do cliente (Obrigatório)" title="Nome do cliente" value='<?php echo $dados['nomeComprador'];?>'  size="40" maxlength=50">
				  <input name="nome" type="text" disabled="disabled" id="nomeCliente" placeholder="Informe o nome do cliente (Obrigatório)" title="Nome do cliente" value='<?php echo $dados['nomeComprador'];?>'  size="40" maxlength=50">
			  </blockquote></td>
			</tr>
			<tr>        
			<tr>
			  <td align="center"><h5>Email:</h5>
				<font color="#000000">
				<blockquote>
				  <input hidden="" name="email" type="Email" id="email" placeholder="Informe o seu email (Obrigatório)" title="Email" value='<?php echo $dados['emailComprador'];?>' size="40" maxlength=50">
				  <input name="emailClie" type="Email" disabled="disabled" id="email" placeholder="Informe o seu email (Obrigatório)" title="Email" value='<?php echo $dados['emailComprador'];?>' size="40" maxlength=50">
			  </blockquote></td>
			  <td align="center"><h5>Nome do Produto</h5>
				<font color="#000000">
				<blockquote>
			  ...
			  <td align="center"><h5>Valor Frete R$: </h5>
				<font color="#000000">
				<blockquote>
				  <input name="precoFrete" type="text" disabled="disabled" id="preco_venda" placeholder="Valor Frete" title="Preço de Frete" value='<?php echo $dados['tipoFrete'];?>' size="20" maxlength="50">
			  </blockquote></td>
			  </td>
			</tr>
			<tr>
			  <td align="center"><h5>* CPF: </h5>
				<font color="#000000">
				<blockquote>
				  <input name="cpf" type="text" disabled="disabled" id="telefone" placeholder="Infome o CPF (Obrigatório)" title="cpf" value='<?php echo $dados['cpfComprador'];?>' size="40" maxlength=50">
			  </blockquote></td>
			  <td align="center"><h5>* Tipo Frete: </h5>
				<font color="#000000">
				<blockquote>
				  <input name="cpf" type="text" disabled="disabled" id="telefone" placeholder="Infome o CPF (Obrigatório)" title="cpf" value='<?php
				  if($dados['tipoFrete'] == 's' || $dados['tipoFrete'] == 'S')
				  {
					  $tipoFrete = 'SEDEX';
				  }
				  else if($dados['tipoFrete'] == 'p' || $dados['tipoFrete'] == 'P')
				  {
					  $tipoFrete = 'PAC';
				  }
				  else if($dados['tipoFrete'] == 'm' || $dados['tipoFrete'] == 'M')
				  {
					  $tipoFrete = 'RET MAOS';
				  }
				  else if($dados['tipoFrete'] == 'v' || $dados['tipoFrete'] == 'V')
				  {
					  $tipoFrete = 'INEXISTENTE';
				  }
				  else
				  {
					  $tipoFrete = 'INEXISTENTE';
			  ...
			  <td align="center"><h5>Código de Rastreio: </h5>
				<font color="#000000">
				<blockquote>
				  <input name="codigoRastreio" type="text" id="Codigo de rastreio" placeholder="Código de rastreio da compra" title="Código de rastreio" value='<?php echo $dados['codigoRastreio'];?>' size="40" maxlength="50">
			  </blockquote></td>
			</tr>
			 </table>
	...
	<hr>
		<h3 class="text-center">PRODUTOS DO CLIÊNTE</h3>
		<hr>
		<div class="container" align="center">
		  <table align="center">
			<tr>
		...
		<?php
			$idComprador = $dados['idComprador'];

			$resultado = $dalCompra->listarHistoricoCompraUsuario($idComprador);
			...
			<tr>
			  ...
			  <td align="center">
				  <?php
					$dados['statusCompra'];
					if($dados['statusCompra'] == 0)
					{
						$status = 'Pendente';
					}
					else if($dados['statusCompra'] == 1)
					{
						$status = 'Aguardando pagamento';
					}
					else if($dados['statusCompra'] == 2)
					{
						$status = 'Em análise';
					}
					else if($dados['statusCompra'] == 3)
					{
						$status = 'Pago';
					}
					else if($dados['statusCompra'] == 4)
					{
						$status = 'Disponível';
					}
					else if($dados['statusCompra'] == 5)
					{
						$status = 'Em disputa';
					}
					else if($dados['statusCompra'] == 6)
					{
						$status = 'Devolvida';
					}
					else if($dados['statusCompra'] == 7)
					{
						$status = 'Cancelada';
					}
					else if($dados['statusCompra'] == 20)
					{
						$status = 'Carrinho Pend PagSeguro';
					}
					else if($dados['statusCompra'] == 21)
					{
						$status = 'Compra Pend PagSeguro';
					}
					else if($dados['statusCompra'] == 25)
					{
						$status = 'Compra Pend Chayds';
					}
					else if($dados['statusCompra'] == 30)
					{
						$status = 'Carrinho Pend Chayds';
					}
